...
class DetectionGeospatial:
    ...
    def assing_tiles(self, gt, preds, num_partitions=3, overlap_percentage = 0):
        # Split into 3 roughly equal partitions
        tile_width = int(self.image_width / num_partitions)
        tile_height = int(self.image_height / num_partitions)

        overlap_x = int(tile_width * overlap_percentage)
        overlap_y = int(tile_height * overlap_percentage)

        x_stride = tile_width - overlap_x
        y_stride = tile_height - overlap_y
